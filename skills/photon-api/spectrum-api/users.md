...
| `GET` | `/users/{userId}/redirect` | Resolve the current shared-line redirect/link-to-chat destination for that user. |
...
Before creating, list or look up existing users so retries do not create duplicate logical recipients. Before deleting, confirm the user and explain that the operation can alter shared-number assignment and access. Never silently recreate a deleted user to work around a routing issue.
